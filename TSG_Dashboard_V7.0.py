"""
TSG_Dashboard_V7.0.py -- Test Suite Generator Dashboard V7.0
V7.0 = V5.0 (full PI/TMO pipeline) + V6 improvements backported:
  - Graceful error handling with fallbacks (try/except around enrichment)
  - Prefix dedup for API paths (removes truncated paths)
  - Before/after dedup count logging
  - Structured section logging with clear markers
  - Enhanced batch resilience with progress reporting on crash
Built on V5.0 (PI/TMO features, batch generation, document provenance,
artifact hash staleness, LLM layer, DB suite storage, humanizer, AI review).

Usage:  streamlit run TSG_Dashboard_V7.0.py
"""
...
# PAGE CONFIG
# ================================================================
st.set_page_config(page_title='TSG V7.0 - AI-Powered Test Suite Generator', page_icon='https://em-content.zobj.net/source/twitter/408/test-tube_1f9ea.png', layout='wide')
...
with left:
    ...
    if not _is_manual_mode:
        ...
        if ss.get('_jira_sync_confirm'):
            ...
            st.warning('⚠️ This will re-fetch Jira data (description, AC, subtasks, comments, attachments) '
                       'for all features and update the local DB. Estimated time: %s.' % _est_jira_time)
    ...
    try:
        ...
        for _ji, (_jfid, _jtitle) in enumerate(_jira_features, 1):
            try:
                ...
                save_jira(jira)
                _subtask_count = len(jira.subtasks) if jira.subtasks else 0
                _comment_count = len(jira.comments) if jira.comments else 0
                _att_count = len(jira.attachments) if jira.attachments else 0
                _jsync_msg('[%d/%d] ✅ %s: %d subtasks, %d comments, %d attachments' % (
                    _ji, _jira_total, _jfid, _subtask_count, _comment_count, _att_count))
                _jira_ok += 1
            ...
            try:
                ...
                for _fi, _current_fid in enumerate(_features_to_run, 1):
                  try:
                    ...
                    print('', flush=True)
                    print('[V7] ═══════════════════════════════════════════════════', flush=True)
                    print('[V7] Block 5: Building test suite for %s' % feature_id, flush=True)
                    print('[V7] ═══════════════════════════════════════════════════', flush=True)
                    ...
                    print('[V7] ═══════════════════════════════════════════════════', flush=True)
                    print('[V7] Block 6: Generating output for %s' % feature_id, flush=True)
                    print('[V7] ═══════════════════════════════════════════════════', flush=True)
                ...
                print('', flush=True)
                print('*' * 50, flush=True)
                print('***   V7 TEST SUITE PREPARED SUCCESSFULLY   ***', flush=True)
