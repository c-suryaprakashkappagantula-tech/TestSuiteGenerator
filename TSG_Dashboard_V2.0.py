    ...
    if not feature_id.strip():
        ...
        with redirect_stdout(logger):
            try:
                ...
                logger.set('Fetching Jira: %s...' % feature_id)
                ...
                if inc_attachments and jira.attachments:
                    logger.set('Downloading %d attachment(s)...' % len(jira.attachments))
                ...
                logger.set('Fetching Chalk: %s...' % ss['selected_pi'])
                ...
                if uploaded_files:
                    for uf in uploaded_files:
                        logger.set('Parsing upload: %s...' % uf.name)
