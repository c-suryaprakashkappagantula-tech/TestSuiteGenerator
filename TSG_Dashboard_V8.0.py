...
with left:
    ...
    expand_matrix = st.checkbox(
        'Full matrix expansion (state x device x channel)', value=False,
        help='Cross eligible Happy-Path TCs across subscriber-state (YL/YD/YP/PY), '
             'device, channel and carrier axes so the suite matches hand-written '
             'matrix suites. Off = additive (one TC per scenario, default).')
    max_expanded = st.slider('Max TCs when expanding', 30, 200, 60, step=10,
                             help='Upper bound; axes are dropped '
                                  '(carrier->channel->device) to stay within this cap, '
                                  'keeping subscriber-state last. ~60 = state coverage; '
                                  '80+ adds the device axis (more TCs).') \
...
with right:
    ...
    if ss['logs']:
        # ── Pipeline status strip — derive stage states from log keywords ──
        def _ps(label, icon, keywords_done, keywords_err):
            for ln in ss['logs']:
                if any(k in ln for k in keywords_err):
                    return '<span class="ps-stage error">%s %s ✗</span>' % (icon, label)
            for ln in ss['logs']:
                if any(k in ln for k in keywords_done):
                    return '<span class="ps-stage done">%s %s ✓</span>' % (icon, label)
            return '<span class="ps-stage pending">%s %s</span>' % (icon, label)

        _strip_html = (
            '<div class="pipeline-strip">'
            + _ps('Jira',    '🔗', ['[JIRA] Found', 'JIRA SYNC'],        ['[JIRA] ERROR', 'JIRA SYNC FAILED'])
            + _ps('Chalk',   '🖍️', ['[CHALK]', 'Chalk sync', 'SYNCED'],  ['[CHALK] ERROR', 'Chalk FAILED'])
            + _ps('Mine',    '⛏️', ['Block 4b', 'Deep mine'],             ['deep mine failed', 'FAILED — Block 4b'])
            + _ps('Engine',  '⚙️', ['Block 5', 'V8 Data-First'],         ['FAILED — Block 5'])
            + _ps('Excel',   '📊', ['[EXCEL]', 'Block 6'],               ['[EXCEL] ERROR', 'FAILED — Block 6'])
            + '</div>'
        )
        ...
        for _line in ss['logs'][-80:]:   # chronological — no reversed()
            ...
            if not _t:
                continue
            _color = '#cbd5e1'           # base raised from #94a3b8
            _icon = '·'
            if 'ERROR' in _t or 'FAIL' in _t:
                ...
                _color = '#fdba74'; _icon = '🗄️'
            elif '═' in _t:
                _static_lines.append('<div style="color:#4f46e5;font-size:11px;margin:4px 0;">%s</div>' % ('─' * 50))
                continue
            _static_lines.append(
                '<div style="padding:2px 0;color:%s;font-size:13px;font-family:\'JetBrains Mono\',monospace;">%s %s</div>' % (
                    _color, _icon, escape(_t)))
        _html = ('<div style="background:#08080f;border:1px solid rgba(99,102,241,0.18);'
                 'border-radius:12px;padding:16px;max-height:450px;overflow-y:auto;">'
                 + ''.join(_static_lines)
                 + '<div id="log-end"></div>'   # anchor — browser stays at bottom on rerun
                 + '</div>')
            ...
            if _review_tcs:
                with st.expander('✏️ Review & Edit Test Cases (%d TCs) — approve before export' % len(_review_tcs),
                    ...
                    st.caption('Edit summaries, change priorities, drop TCs. Edits persist per feature. '
                               'Click "Apply & Export" to regenerate the Excel with your overrides applied.')
                    ...
                    with _rev_col1:
                        if st.button('✅ Apply & Export', use_container_width=True, key='btn_apply_overrides'):
                            try:
                                from modules.database import save_tc_overrides, load_tc_overrides
                                from modules.excel_generator import generate_excel

                                # 1. Persist override choices to DB (indexed by position since sno may be empty)
                                _overrides = []
                                for _oi, (_, row) in enumerate(_edited.iterrows()):
                                    _sno = row.get('sno', '') or str(_oi + 1)
                                    _overrides.append({
                                        'tc_sno': _sno,
                                ...
                                # 2. Apply overrides to the live suite object
                                _suite_obj = ss.get('_suite_obj')
                                if not _suite_obj:
                                    st.error('Suite object not available — please re-generate the feature first.')
                                else:
                                    # Build a lookup: position-index → override row
                                    _ov_by_idx = {str(i): ov for i, ov in enumerate(_overrides)}
                                    _kept_tcs = []
                                    for _ti, tc in enumerate(_suite_obj.test_cases):
                                        ov = _ov_by_idx.get(str(_ti))
                                        if ov and ov['action'] == 'drop':
                                            continue  # drop this TC
                                        if ov and ov['action'] in ('keep', 'edit'):
                                            if ov.get('edited_summary'):
                                                tc.summary = ov['edited_summary']
                                            if ov.get('priority_override'):
                                                tc.priority = ov['priority_override']
                                        _kept_tcs.append(tc)
                                    ...
                                    _filtered_suite = _copy.copy(_suite_obj)
                                    _filtered_suite.test_cases = _kept_tcs

                                    # 5. Re-generate Excel with a new filename
                                    _new_path = generate_excel(_filtered_suite)
                                    ss['result_path'] = str(_new_path)
                                    ss['suite_info'] = dict(ss.get('suite_info') or {})
                                    ss['suite_info']['tc_count'] = len(_kept_tcs)

                                    st.success('✅ Exported: %d kept, %d dropped → %s' % (
                                        len(_kept_tcs), len(_suite_obj.test_cases) - len(_kept_tcs),
                                        _new_path.name))
                                    st.rerun()
                            except Exception as _ov_err:
                                import traceback
                                st.error('Apply failed: %s' % str(_ov_err)[:200])
...
class LiveLog:
    ...
    def _format_line(self, text):
        ...
        icon = '·'
        color = '#cbd5e1'  # base raised from #94a3b8 for readability

        if '═' in t:
            return '<div style="color:#4f46e5;font-size:11px;margin:4px 0;">%s</div>' % ('─' * 50)
        ...
        # Color coding — brighter semantic palette
        ...
        return '<div style="padding:2px 0;color:%s;font-size:13px;font-family:\'JetBrains Mono\',monospace;">%s %s</div>' % (
    ...
    def write(self, s):
        ...
        html = (
            "<div style='background:#08080f;border:1px solid rgba(99,102,241,0.18);"
            "border-radius:12px;padding:16px;max-height:450px;overflow-y:auto;"
