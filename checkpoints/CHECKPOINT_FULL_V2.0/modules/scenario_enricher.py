...
def enrich_scenarios(test_cases, feature_id, feature_context, log=print):
    ...
    # ── Cap at 50% of existing (Point 14: reserve slots for high-priority layers) ──
    cap = max(5, int(n * 0.5))
    if len(new_tcs) > cap:
        # Prioritize: E2E > Verification (L8) > Field Validation (L3)
        HIGH_PRIORITY_KEYWORDS = ['end-to-end', 'e2e', 'transaction history', 'century',
                                  'portal', 'nbop', 'audit']
        high_prio = [tc for tc in new_tcs if any(kw in tc.summary.lower() for kw in HIGH_PRIORITY_KEYWORDS)]
        low_prio = [tc for tc in new_tcs if tc not in high_prio]
        # Take all high-priority first, then fill remaining slots with low-priority
        new_tcs = high_prio[:cap] + low_prio[:max(0, cap - len(high_prio))]
        new_tcs = new_tcs[:cap]
        log('[ENRICH]   Capped at %d (50%% of %d) — high-priority layers preserved' % (cap, n))
