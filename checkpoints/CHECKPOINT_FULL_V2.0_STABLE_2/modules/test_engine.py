...
def _chalk_scenario_to_tc(sc, idx, feature_id):
    ...
    if sc.validation and sc.validation != sc.title:
        description = 'To validate %s. Expected: %s' % (clean_title.rstrip('.'), sc.validation[:200])
    else:
        description = 'To validate %s' % clean_title.rstrip('.')
...
def _clean_tc_title(raw_title, feature_id):
    ...
    # If it's a long paragraph (>100 chars), extract first sentence
    if len(t) > 100:
    ...
    for tc in expandable:
        for combo in combos:
            ...
            new_tc = TestCase(
                ...
                description='%s\nDevice: %s | Channel: %s | OS: %s | SIM: %s | Network: %s' % (
                    tc.description, combo['device'], combo['channel'],
