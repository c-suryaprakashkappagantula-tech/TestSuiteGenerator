"""Check 4009 preconditions for sync scenarios."""
import sys, os, json
...
print()
# Show first 10 TCs with preconditions
for tc in suite.test_cases[:10]:
    name = tc.summary.split('_', 2)[-1][:70] if '_' in tc.summary else tc.summary[:70]
    print('TC%s: %s' % (tc.sno, name))
    precon_lines = tc.preconditions.split('\n')
    for pl in precon_lines[:3]:
        print('  %s' % pl[:80])
    print()
